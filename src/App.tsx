  ...
  return (
    <div className="min-h-screen bg-bg text-white selection:bg-primary selection:text-bg cursor-none">
      ...
      <main className="container mx-auto px-6 pt-32 pb-20 space-y-40">
        {/* Hero Section */}
        <section id="home" className="min-h-[80vh] flex flex-col justify-center items-center text-center relative">
          <motion.div
          ...
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            transition={{ delay: 1 }}
            className="absolute bottom-0 left-1/2 -translate-x-1/2 flex gap-8"
          >
        ...
        {/* Skills Section */}
        <section id="skills">
          <SectionHeader title="TECH STACK" subtitle="Core Competencies" center />
        ...
        {/* Journey Section */}
        <section id="journey">
          <SectionHeader title="JOURNEY" subtitle="Experience & Education" center />
                  ...
                  <div className="w-full md:w-1/2">
                    <Tilt tiltMaxAngleX={5} tiltMaxAngleY={5}>
                      <div className="glass p-8 rounded-3xl hover:bg-white/5 transition-all duration-300 border border-white/5 hover:border-primary/30 group">
                        <span className="text-primary font-mono text-sm mb-2 block group-hover:neon-text transition-all">{item.year}</span>
                        <h3 className="text-xl font-bold mb-1 group-hover:text-primary transition-colors">{item.title}</h3>
                        <p className="text-secondary font-medium mb-4">{item.organization}</p>
                        <p className="text-white/60 text-sm leading-relaxed mb-4">{item.description}</p>
                        {item.technologies && (
                          <div className="flex flex-wrap gap-2">
                            {item.technologies.map((tech, j) => (
                              <span key={j} className="text-[10px] uppercase tracking-widest text-white/40 px-2 py-1 bg-white/5 rounded border border-white/5 group-hover:border-primary/20 transition-colors">
                                {tech}
                              </span>
                            ))}
                          </div>
        ...
        <section id="projects" className="space-y-20">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              {portfolioData.projects.filter(p => p.type === 'professional').map((project, i) => (
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {portfolioData.projects.filter(p => p.type === 'personal').map((project, i) => (
                <ProjectCard key={i} project={project} index={i} />
              ))}
            </div>
          </div>
        ...
        {/* GitHub Activity */}
        <section id="github">
          <SectionHeader title="ACTIVITY" subtitle="GitHub Contributions" center />
          <div className="glass p-8 rounded-3xl flex flex-col items-center overflow-x-auto">
            ...
            <div className="grid md:grid-cols-2 gap-8 mt-12 w-full">
              <img src={portfolioData.github.stats} alt="GitHub Stats" className="w-full rounded-xl" />
              <img src={portfolioData.github.languages} alt="Top Languages" className="w-full rounded-xl" />
            </div>
          </div>
        </section>
        ...
        <section id="contact" className="max-w-4xl mx-auto">
          <SectionHeader title="CONTACT" subtitle="Let's Build Something" center />
          <div className="grid md:grid-cols-3 gap-8 mb-12">
            <div className="glass p-6 rounded-3xl text-center flex flex-col items-center gap-4 group hover:border-primary/30 transition-all">
              ...
              <div>
                <h4 className="font-orbitron text-xs text-white/40 uppercase tracking-widest mb-1">Email</h4>
                <p className="text-sm font-mono">{portfolioData.profile.email}</p>
              </div>
            </div>
            <div className="glass p-6 rounded-3xl text-center flex flex-col items-center gap-4 group hover:border-secondary/30 transition-all">
              ...
              <div>
                <h4 className="font-orbitron text-xs text-white/40 uppercase tracking-widest mb-1">LinkedIn</h4>
                <p className="text-sm font-mono">/in/manishtiwari</p>
              </div>
            </div>
            <a 
              href={portfolioData.socials.find(s => s.platform === 'WhatsApp')?.url}
              target="_blank"
              rel="noreferrer"
            ...
            >
              ...
              <div>
                <h4 className="font-orbitron text-xs text-white/40 uppercase tracking-widest mb-1">WhatsApp</h4>
          ...
          <div className="glass p-10 rounded-[3rem] relative overflow-hidden">
            ...
            <form className="space-y-8 relative z-10">
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-xs font-orbitron text-white/40 uppercase tracking-widest">Full Name</label>
                  <input type="text" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 focus:outline-none focus:border-primary/50 transition-colors" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-orbitron text-white/40 uppercase tracking-widest">Email Address</label>
                  <input type="email" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 focus:outline-none focus:border-primary/50 transition-colors" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-xs font-orbitron text-white/40 uppercase tracking-widest">Message</label>
                <textarea rows={5} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 focus:outline-none focus:border-primary/50 transition-colors resize-none" />
              </div>
              <button className="w-full py-5 bg-primary text-bg font-orbitron font-black text-lg rounded-2xl shadow-[0_0_30px_rgba(0,255,65,0.3)] hover:scale-[1.02] transition-transform active:scale-95">
                SEND TRANSMISSION
              </button>
            </form>
          </div>
      ...
      <footer className="py-10 border-t border-white/5 text-center text-white/40 font-mono text-xs">
        ...
        <p className="mt-2 text-primary/40">BUILT WITH REACT, THREE.JS & NEON DREAMS</p>
      </footer>
...
const Navbar = () => {
  ...
  return (
    <nav className={cn(
      ...
      scrolled ? "bg-bg/80 backdrop-blur-md border-b border-white/5 py-3" : "bg-transparent"
    )}>
      <div className="container mx-auto flex items-center justify-between">
        <a href="#home" className="font-orbitron text-2xl font-black tracking-tighter flex items-center gap-2">
          <div className="w-8 h-8 bg-primary rounded flex items-center justify-center text-bg text-sm">MT</div>
      ...
      <div className="glass rounded-3xl overflow-hidden group border border-white/5 hover:border-primary/30 transition-all duration-500">
        <div className="relative h-64 overflow-hidden">
          ...
          <div className="absolute inset-0 bg-bg/80 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-6">
            {project.github && (
              <a href={project.github} target="_blank" rel="noreferrer" className="p-4 bg-primary/20 rounded-full text-primary hover:bg-primary hover:text-bg transition-all">
                <Github size={24} />
              </a>
            )}
            {project.demo && (
              <a href={project.demo} target="_blank" rel="noreferrer" className="p-4 bg-secondary/20 rounded-full text-secondary hover:bg-secondary hover:text-bg transition-all">
                <ExternalLink size={24} />
              </a>
            )}
            {!project.github && !project.demo && (
              <span className="text-white/40 font-orbitron text-xs uppercase tracking-widest">Internal Project</span>
            )}
          </div>
          {/* Metrics Overlay */}
          <div className="absolute bottom-4 left-4 right-4 flex gap-2">
            {project.metrics?.map((metric: any, idx: number) => (
              <div key={idx} className="bg-black/60 backdrop-blur-md border border-white/10 px-3 py-1 rounded-lg">
                <p className="text-[8px] text-white/40 uppercase font-orbitron">{metric.label}</p>
        ...
        <div className="p-8">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className={cn(
              ...
              project.type === 'professional' ? "border-primary/30 text-primary bg-primary/5" : "border-secondary/30 text-secondary bg-secondary/5"
            )}>
              {project.type}
            </span>
          </div>
          <p className="text-white/60 text-sm mb-6 leading-relaxed">{project.description}</p>
          <div className="flex flex-wrap gap-2">
            {project.tags.map((tag: string, j: number) => (
              <span key={j} className="text-[10px] font-mono text-white/40 px-2 py-1 bg-white/5 border border-white/10 rounded group-hover:border-primary/20 transition-colors">
                {tag}
              </span>
            ))}
          </div>
...
const SectionHeader = ({ title, subtitle, center = false }: { title: string, subtitle: string, center?: boolean }) => (
  <div className={cn("mb-16", center ? "text-center" : "")}>
    ...
    <div className={cn("h-1 bg-gradient-to-r from-primary to-transparent mt-4", center ? "mx-auto w-24" : "w-20")} />
  </div>
);
...
const ScrollProgress = () => {
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-1 z-[100]">
      <div className="h-full bg-primary shadow-[0_0_10px_#00FF41]" style={{ width: `${progress}%` }} />
    </div>
  );
};
